...
pub(crate) fn find_all(current_page: i64, other_source: ImageSource) -> anyhow::Result<Page> {
    ...
    let total = bing
        .count()
        .filter(source.eq(other_source.to_string()))
...
pub(crate) fn insert(wallpaper_vec: Vec<NewBing>) -> bool {
    ...
    let need_wallpaper_vec = wallpaper_vec
        .iter()
        .filter(|item| wallpaper_all.iter().any(|x| x.url == item.url) == false)
        .map(|x| x.clone())
        .collect::<Vec<_>>();
    let mut flag = true;
    ...
    diesel::insert_into(bing::table())
        .values(&need_wallpaper_vec)

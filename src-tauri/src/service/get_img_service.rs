...
lazy_static! {
    static ref SPOTLIGHT_UHD_PATH: String = format!("{}{}",  get_home(), "/wallpaper/spotlight/images/uhd/");
    static ref SPOTLIGHT_NORMAL_PATH: String = format!("{}{}", get_home(), "/wallpaper/spotlight/images/normal/");
...
impl ImageTrait for BingImage {
    async fn get_totals(&self) -> anyhow::Result<i32, Error> {
        let client = reqwest::Client::new();
         match  client.get("https://peapix.com/bing/cn").timeout(*HTTP_TIME_OUT).send().await{
             Ok(res) => {
    ...
    async fn get_page(&self, page_size:i32) -> anyhow::Result<Vec<NewBing>, Error> {
        ...
        if let Ok(res) = client.get(format!("https://peapix.com/bing/cn?page={}", &page_size)).timeout(*HTTP_TIME_OUT).send().await {
            if res.status() == StatusCode::OK {
...
impl ImageTrait for WallpaperImage {
    ...
    async fn get_page(&self, page:i32) -> anyhow::Result<Vec<NewBing>, Error> {
        ...
        if let Ok(res) = client.get(format!("{}/girls/?page={}", WALL_URL.to_string(), &page)).timeout(*HTTP_TIME_OUT).send().await {
            if res.status() == StatusCode::OK {
                let data = res.text().await?;
                let html = htmler::Html::parse_fragment(&data);

...
pub async fn translate(q: String) -> anyhow::Result<String> {
    ...
    let sign = || {
        let sign_str =
            format!("client={client}&mysticTime={mystic_time}&product={product}&key={key}");
        ...
        .await
    {
        ...
        let vec = engine::GeneralPurpose::new(&alphabet::URL_SAFE, general_purpose::PAD)
            .decode(str.trim_end())
            .unwrap();
...
mod tests {
    ...
    async fn yd_translate_test() {
        for _ in 0..1 {
            if let Ok(res) = translate(
                "Lake Lago Pehoe and Los Cuernos, Torres del Paine National Park, Chile, Patagonia"
                    .to_string(),
            )
            .await
            {
